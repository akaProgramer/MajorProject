...
import sys
from PyQt5.QtWidgets import QApplication, QFileDialog, QInputDialog, QLabel, QLineEdit, QMainWindow, QFrame, QPushButton, QComboBox
...
class App(QMainWindow):
    ...
    def select_file_path(self):
        self.path_fileName, _ = QFileDialog.getOpenFileName(
            self, "Select File", "", "All Files (*);;JPEG  (*.jpeg);;JPG  (*.jpg);;PNG  (*.png)")
    ...
    def select_file_path_convertor(self):
        self.convert_path, _ = QFileDialog.getOpenFileName(
            self, "Select File", "", "All Files (*);;JPEG  (*.jpeg);;JPG  (*.jpg);;PNG  (*.png)")
        self.image_conv_path.setText(self.convert_path)

    def convertor(self):
        convertor_object = converter.Convert(
            self.convert_path, self.convert_path, True)
    ...
    def multi_convertor(self):
        ...
        print(self.source_folder_conv)
        convertor_object = converter.Convert(
            self.source_folder_conv, self.source_folder_conv_, False)
        option = self.extension_choosen()
        print(option)
        try:
            if option == 1:
    ...
    # -----------------------multi image expanded------------------------------------
        ...
        self.multi_img_conv_para.setText(
            "Click here to convert multiple images \n       by selecting the whole folder")
    ...
    # -----------------------multi image expanded------------------------------------
        ...
        self.browse_button_conv_f.clicked.connect(
            self.select_source_folder_conv)
